# Ryan sheet polish — progress

Setlist order = `src/data/setlist.json` ids. Check off when the Ryan sheet is stage-ready
(ear-checks done, scroll dialed, no known placement bugs). Songs without a `.ryan.txt` yet
stay unchecked until one exists and is polished.

## Status

- [ ] 01-welcome-home
- [ ] 02-all-the-small-things
- [ ] 03-a-little-less-conversation
- [ ] 04-i-believe-in-a-thing-called-love
- [ ] 05-thunderstruck
- [ ] 06-dont-stop-believin
- [ ] 07-tribute — **in progress (2026-07-22):** intro + fills ^1/^2/^3, pre-ROCK chords stripped, 4Amber→ETHEREAL, appendix/tab stitch cut. Still open: ear-check fill ^3 on "men"; dial scrollSpeed on /dev/; reload amp slot 4. Research links in `practice-notes-worksheet.md` §07.
- [ ] 08-while-my-guitar-gently-weeps
- [ ] 09-pride-and-joy
- [ ] 10-zombie
- [ ] 11-hunger-strike
- [ ] 12-s-o-b
- [ ] 13-save-a-horse-ride-a-cowboy
- [ ] 14-sweet-home-alabama
- [ ] 15-dani-california
- [ ] 16-teenage-dirtbag
- [ ] 17-the-pretender
- [ ] 18-here-it-goes-again
- [ ] 19-purple-rain
- [ ] 20-when-i-come-around
- [ ] 21-the-middle — has `.ryan.txt` (A-string fret chips); not polished yet
- [ ] 22-dream-on
- [ ] 23-lola-montez
- [ ] 24-mary-janes-last-dance
- [ ] 25-valerie
- [ ] 26-voodoo-child-slight-return
- [ ] 27-fat-bottomed-girls
- [ ] 28-mama-i-m-coming-home
- [ ] 29-ain-t-goin-down-til-the-sun-comes-up
- [ ] 30-pink-pony-club
- [ ] 31-banditos

## Last done

Tribute pass underway (fill cues + amp retune + trim). Next after Tribute closes: **21-the-middle**.
