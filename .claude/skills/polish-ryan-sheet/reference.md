...
even on phone). Lyricless runs of up to 8 chips stay on one row (Banditos A-E walk,
FBG D×5); longer merged runs still pack 4-across. For 3-chord loops (Sweet Home `D C G`),
write each cycle as one multi-chord line (`D C G` then blank then `D C G`) — parser only
auto-merges *single*-chord instrumental lines, so authored cycles stay separate rows (no `·`
